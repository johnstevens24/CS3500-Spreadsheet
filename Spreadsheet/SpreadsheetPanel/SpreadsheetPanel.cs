...
// Modified by John Stevens Oct 2021
...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        /// <summary>
        /// Sets whether or not to draw the dependents
        /// </summary>
        /// <param name="b">boolean whether to draw or not</param>
        public void setDrawDependents(bool b)
        {
            drawingPanel.setDrawDependents(b);
        }

        /// <summary>
        /// Sets the dependents coordinates
        /// </summary>
        /// <param name="x">array of x coords</param>
        /// <param name="y">array of y coords</param>
        public void setDependentsCoords(int[] x, int[] y)
        {
            drawingPanel.setDependentsCoords(x, y);
        }
        ...
        protected override void OnResize(EventArgs eventargs)
        {
            ...
            if (FindForm() == null || FindForm().WindowState != FormWindowState.Minimized)
            {
                ...
                vScroll.LargeChange = Math.Max(0,(Height - SCROLLBAR_WIDTH) / DATA_ROW_HEIGHT);
        ...
        {
            //these are used for the draw dependents function
            private bool drawDependents = false;
            private int[] x;
            private int[] y;
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                //redraws dependent cells' borders in red. Used for Show Dependents feature
                if (drawDependents && x != null)
                {
                    //Change the color of the pen to red
                    ...
                    for (int i = 0; i < x.Length; i++)
                    {
                        ...
                        if ((_selectedCol - _firstColumn >= 0) && (_selectedRow - _firstRow >= 0))
                        {
                            //same from down below, it just uses the x and y values passed in via int arrays
                            e.Graphics.DrawRectangle(
                                pen,
                                new Rectangle(LABEL_COL_WIDTH + (x[i] - _firstColumn) * DATA_COL_WIDTH + 1,
                                              LABEL_ROW_HEIGHT + (y[i] - _firstRow) * DATA_ROW_HEIGHT + 1,
                                              DATA_COL_WIDTH - 2,
                                              DATA_ROW_HEIGHT - 2));
                        }
                    }
                    //Change the pen color back to black
