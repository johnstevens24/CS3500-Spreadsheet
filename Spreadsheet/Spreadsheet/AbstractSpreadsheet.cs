...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        private void Visit(string start, string name, ISet<string> visited, LinkedList<string> changed)
        {
            ...
            visited.Add(name);
            //gets a list of the current cell's dependents
            foreach (string n in GetDirectDependents(name))
            {
                //if n equals the cell we started at, throw a circular exception
                if (n.Equals(start))
                {
                    throw new CircularException();
                } //if it doesn't equal start, and it isn't contained in the visited Hashset, re-call this method using it as the second string parameter
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            //finally, add the original cell as the first in the set
